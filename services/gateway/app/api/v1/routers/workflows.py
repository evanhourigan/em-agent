...
from ....schemas.workflows import WorkflowRunRequest, WorkflowRunResponse, WorkflowJobResponse
...
def run_workflow(
    ...
    try:
        ...
        if not action:
            ...
            if settings.opa_url:
                try:
                    ...
                    logger.warning("workflow.opa_http_error", error=str(e), status_code=e.response.status_code)
                    action = None
        ...
        if action == "block":
            ...
            if global_metrics:
                try:
                    global_metrics["workflows_auto_vs_hitl_total"].labels(mode="hitl").inc()
        ...
        logger.info("workflow.queued", id=log.id, rule=rule, subject=subject, action=action)
...
def get_job(id: int, session: Session = Depends(get_db_session)) -> WorkflowJobResponse:
    ...
    try:
        ...
        logger.error("workflow.get_job.db_error", error=str(e), job_id=id, exc_info=True)
        raise HTTPException(status_code=503, detail="Database unavailable")
    except Exception as e:
        logger.error("workflow.get_job.unexpected_error", error=str(e), job_id=id, exc_info=True)
        raise HTTPException(status_code=500, detail="Internal server error")
