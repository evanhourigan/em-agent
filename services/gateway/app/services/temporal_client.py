...
class TemporalGateway:
    ...
    async def ensure(self) -> Optional[Client]:
        if not _HAS_TEMPORAL:
            return None
        if self._client is None:
            self._client = await Client.connect(self._addr, namespace=self._namespace)
            self._logger.info("temporal.connected", address=self._addr, namespace=self._namespace)
        return self._client


_gw: Optional[TemporalGateway] = None
